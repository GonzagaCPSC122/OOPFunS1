...
class Pet(Animal): # Pet is-a Animal (it inherits from Animal)
    """Docstring for Pet
    """
    def __init__(self, species: str, age: int, name: str = "default name") -> None:
        super().__init__(species, age) # invoke Animal's initiazer
        # (let Animal initialize Animal-specific state)
        self._name = name # Pet initializes Pet-specific state
...
class WildAnimal(Animal): # WildAnimal is-a Animal (it inherits from Animal)
    """Docstring for WildAnimal
    """
    def __init__(self, species: str, age: int, noise: str = "default noise") -> None:
        super().__init__(species, age) # invoke Animal's initiazer
        # (let Animal initialize Animal-specific state)
        self._ferocious_noise = noise # WildAnimal initializes WildAnimal-specific state
...
def main():
    ...
    p1 = Pet("dog", 5, "spike")
    print(p1)
    w1 = WildAnimal("lion", 10, "roooooar!!!")
    print(w1)
    print()
